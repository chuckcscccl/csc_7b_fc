...
pub fn eval(e:&Expr) -> Option<i32>
{
   match e {
     ...
     Divide(x,y) => eval(y).and_then(|b|{
                      if b!=0 {eval(x).map(|a|a/b)}
                      else {None}
                    }),
     Mod(x,y) =>    eval(y).and_then(|b|{
                      if b!=0 {eval(x).map(|a|a%b)}
                      else {None}
                    }),
     _ => None,
...
pub fn lex(inp:&str) -> Vec<Expr>
{
   ...
   while i<input.len()
   {
      let mut c = input[i]; i+=1; // get one char
      let mut isnum=false;
      let mut n = 0;
	 ...
	 if i<input.len() {
      ...
      if isnum {tokens.push(Val(n as i32));}
      if !c.is_digit(10) && c!=' ' {tokens.push(Sym(c));}
   }//while
   tokens.push(EOF);
   return tokens;
}//lexer
...
{ 
  ...
  {
     let sl = stack.len();
     match stack.as_slice() {   // match against stack as slice
